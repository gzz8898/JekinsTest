...
public class DocumentUtil {
    ...
    public static boolean mkdirs(Context context, File dir) {
        ...
        if (!res) {
            if (DocumentUtil.isOnExtSdCard(dir, context)) {
                DocumentFile documentFile = DocumentUtil.getDocumentFile(dir, true, context);
    ...
    public static boolean delete(Context context, File file) {
        ...
        if (!ret && DocumentUtil.isOnExtSdCard(file, context)) {
            DocumentFile f = DocumentUtil.getDocumentFile(file, false, context);
            if (f != null) {
                ret = f.delete();
            }
        }
    ...
    public static boolean canWrite(Context context, File file) {
        ...
        if (!res && DocumentUtil.isOnExtSdCard(file, context)) {
            DocumentFile documentFile = DocumentUtil.getDocumentFile(file, true, context);
            res = documentFile != null && documentFile.canWrite();
        }
        return res;
    }
    ...
    public static InputStream getInputStream(Context context, File destFile) {
        ...
        try {
            if (!canWrite(destFile) && isOnExtSdCard(destFile, context)) {
                DocumentFile file = DocumentUtil.getDocumentFile(destFile, false, context);
    ...
    public static boolean checkWritableRootPath(Context context, String rootPath) {
        ...
        if (!root.canWrite()) {
            ...
            if (DocumentUtil.isOnExtSdCard(root, context)) {
                DocumentFile documentFile = DocumentUtil.getDocumentFile(root, true, context);
